...
export default function UnifiedAuthDemo({ user }: UnifiedAuthDemoProps) {
  ...
  return (
    <AuthDemoPage
      ...
      intro="One unified page — use email & password or continue with Google. Supabase handles the session; onAuthStateChange keeps the UI in sync instantly."
      steps={[
        "Toggle between Sign up and Sign in.",
        "Submit with email/password or click Continue with Google.",
        "Watch the session panel hydrate in real time.",
        "Sign out to reset.",
      ]}
    >
      {!currentUser && (
        ...
        >
          <div
            className="pointer-events-none absolute -left-6 -top-6 -z-10 h-24 w-32 rounded-full bg-[radial-gradient(circle,_rgba(66,133,244,0.2),_transparent)] blur-xl"
            aria-hidden="true"
          />
          <div
            className="pointer-events-none absolute -bottom-8 right-0 -z-10 h-28 w-40 rounded-full bg-[radial-gradient(circle,_rgba(16,185,129,0.15),_transparent)] blur-xl"
            aria-hidden="true"
          />
              ...
              {(["signup", "signin"] as Mode[]).map((option) => (
                <button
                  ...
                  className={`rounded-full px-4 py-1 transition ${
                    mode === option
                      ? "bg-indigo-500/30 text-white shadow shadow-indigo-500/20"
                      : "text-slate-400"
                  }`}
                >
          ...
          <button
            type="submit"
            disabled={loading}
            className="mt-5 inline-flex w-full items-center justify-center rounded-full bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-900/40 transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-50"
          >
